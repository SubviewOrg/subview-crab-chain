defmodule Explorer.Chain.SmartContract do
  ...
  * `bytecode_checked_at` - timestamp of the last check of contract's bytecode matching (DB and BlockChain)
  ...
  def add_submitted_comment(contract_lines, inserted_at) when is_list(contract_lines) do
    ...
    subview_index =
      Enum.find_index(contract_lines, fn line ->
        String.contains?(line, "Submitted for verification at subview.com")
      end)

    cond do
      etherscan_index && subview_index ->
      ...
      etherscan_index && !subview_index ->
        List.replace_at(
          contract_lines,
          etherscan_index,
          "* Submitted for verification at subview.com on #{inserted_at}"
        )

      !etherscan_index && !subview_index ->
        header = ["/**", "* Submitted for verification at subview.com on #{inserted_at}", "*/"]
